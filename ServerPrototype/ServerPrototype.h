      ...
      while (true) {
		  try {
			  ...
			  if (msg.to().port != msg.from().port) {// avoid infinite message loop			  
				  //Sends multiple files in case of multiple files selected in checkout
				  if (msg.command() == "checkOut")  {
					  std::string files = reply.value("fileName");
					  std::vector<std::string> fileList = Utilities::split(files, ';');
					  for (std::string file : fileList)  {
						  reply.attribute("sendingFile", file);
						  postMessage(reply);
